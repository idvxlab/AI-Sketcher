...
def refine(s):
    ...
    for i in s:
      if np.random.rand(1)>0.9:
        i+=[2*np.random.rand(1)[0]-1,2*np.random.rand(1)[0]-1,0]
      l.append(i)
...
class DataLoader(object):
    ...
    def _get_batch_from_indices(self, indices):
        ...
        for idx in range(len(indices)):
            ...
            label_copy = np.copy(self.label[i])
            # image_copy = np.copy(self.image[i])
            if self.augment_stroke_prob > 0:
                data_copy = augment_strokes(data_copy, self.augment_stroke_prob)
            x_batch.append(data_copy)
            length = len(data_copy)
            seq_len.append(length)
            x_label.append(label_copy)
            # x_image.append(image_copy)
        seq_len = np.array(seq_len, dtype=int)
        # We return three things: stroke-3 format, stroke-5 format, list of seq_len.
        return x_batch, self.pad_batch(x_batch, self.max_seq_length), seq_len, x_label #, x_image

    def random_batch(self):
        """Return a randomised portion of the training data."""
        idx = np.random.permutation(range(0, len(self.strokes)-100))[0:self.batch_size]
        #idx = np.random.permutation(range(0, len(self.image)))[0:self.batch_size]
