...
def orthogonal(shape):
    ...
    flat_shape = (shape[0], np.prod(shape[1:]))  #[shape[0], shape[1]*...*shape[n]]取出当前1D array的第一个元素，并把之后的元素连乘
    a = np.random.normal(0.0, 1.0, flat_shape)  #生成一个[shape[0], shape[1]*...*shape[n]]
    u, _, v = np.linalg.svd(a, full_matrices=False)  #a(m,n) --> svd u(m,k),vT(k,n) k=min(m,n)
    q = u if u.shape == flat_shape else v  #取出形状与flat_shape形状相同的正交矩阵
    return q.reshape(shape)

#将正交矩阵的生成封装为一个生成tf_constant的函数
def orthogonal_initializer(scale=1.0):
    """Orthogonal initializer."""
    def _initializer(shape, dtype=tf.float32,
                   partition_info=None):  # pylint: disable=unused-argument
        return tf.constant(orthogonal(shape) * scale, dtype) #Creates a constant tensor.

    return _initializer  #e.g.: orthogonal_initializer()(shape) 获得格式为tf.constant的正交矩阵-->tensorflow initializer


#生成shape为(unit,unit*4)，范围在(-1,1)的随机正交矩阵
def lstm_ortho_initializer(scale=1.0):
    """LSTM orthogonal initializer."""
    def _initializer(shape, dtype=tf.float32,
                   partition_info=None):  # pylint: disable=unused-argument
        size_x = shape[0]
        size_h = shape[1] // 4  # assumes lstm. 对4整除
        t = np.zeros(shape)
        #每 size_h 列进行正交矩阵生成，生成t的形状与shape相同，之所以设置为4是因为lstm中有4个权重矩阵需要更新
...
class LSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            #state = tf.concat([new_c, new_h], 1)
            c, h = tf.split(state, 2, 1)  #c表示LTM，h表示STM
            ...
            #i --> W_i*[STM_{t-1},x_t]+b_i 短期记忆-遗忘矩阵
            #j --> W_j*[STM_{t-1},x_t]+b_j 短期记忆-记忆矩阵
            #f --> W_f*[STM_{t-1},x_t]+b_f 长期记忆-遗忘矩阵
            #o --> W_o*[STM_{t-1},x_t]+b_o 短期记忆-融合矩阵
            ...
            #new LTM 
            #remember gate = forget gate + learn gate
            """
                c --> LTM_{t-1} 
                f --> W_f*[STM_{t-1},x_t]+b_f
                forget gate:
                    c * sigmoid(f+bias)
                    
                i --> W_i*[STM_{t-1},x_t]+b_i
                g --> tanh(W_j*[STM_{t-1},x_t]+b_j)
                learn gate:
                    g * sigmoid(i)
            ...
            #use gate
            """
                o --> W_o*[STM_{t-1},x_t]+b_o
            """
            #这里有个问题，在lstm中new_STM = tanh(LTM_{t-1},b) + sigmoid(o) 
            #但此处直接使用了tanh(new_LTM)+ sigmoid(o),这样可以吗？ 可以
...
def layer_norm_all(h,
    ...
    rstd = tf.rsqrt(var + epsilon) #因为是倒数所以要防止它为0
    h_reshape = (h_reshape - mean) * rstd #rstd表示标准差的倒数，是因为乘法算得比除法快吗。。？
...
class LayerNormLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, x, state, timestep=0, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            x_size = x.get_shape().as_list()[1]
            batch_size = x.get_shape().as_list()[0] #把x的第一维当作batch_size
            ...
            concat = layer_norm_all(concat, batch_size, 4, h_size, 'ln_all') #layer_norm_all
            ...
            new_c = c * tf.sigmoid(f + self.forget_bias) + tf.sigmoid(i) * g
            new_h = tf.tanh(layer_norm(new_c, h_size, 'ln_c')) * tf.sigmoid(o) #layer_norm
...
class HyperLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def hyper_norm(self, layer, scope='hyper', use_bias=True):
        ...
        # batch-normalize the hidden-to-hidden transition
        init_gamma = 0.10  # cooijmans' da man.
        with tf.variable_scope(scope):
            zw = super_linear(  #super_linear
                self.hyper_output,
                embedding_size,
                init_w='constant',
                weight_start=0.00,
    ...
    def __call__(self, x, state, timestep=0, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            h = total_h[:, 0:self.num_units]
            c = total_c[:, 0:self.num_units] #提取出大网络的LTM和STM
            self.hyper_state = tf.concat(
              [total_h[:, self.num_units:], total_c[:, self.num_units:]], 1) #提取出hyperNetwork的LTM和STM
