...
tf.app.flags.DEFINE_integer('expr_num', 14,
                            """How many classes of emotions.""")
...
def load_dataset(data_dir, model_params, inference_mode=False):
    ...
    for dataset in datasets:
        ...
        if data_dir.startswith('http://') or data_dir.startswith('https://'):
            ...
            # print(np.genfromtxt(response.content))
            #
            # print(StringIO(response.content))
            data = np.load(StringIO(response.content))
            data = np.load(str(response.content))
            # pass
        else:
            if six.PY3:
        ...
        tf.logging.info('Loaded {}/{}/{}/{}/{}/{} from {}'.format(
            len(data['train']), len(data['valid']), len(data['test']),len(data['train_label']), len(data['valid_label']), len(data['test_label']),
            dataset))
        if train_strokes is None:
            ...
            train_label = np.concatenate((train_label, data['train_label']))
            valid_label = np.concatenate((valid_label, data['valid_label']))
            test_label = np.concatenate((test_label, data['test_label']))

            # train_image = np.concatenate((train_image, data['train_image']))
            # valid_image = np.concatenate((valid_image, data['valid_image']))
            # test_image = np.concatenate((test_image, data['test_image']))
...
def evaluate_model(sess, model, data_set):
    ...
    total_r_cost = 0.0
    total_kl_cost = 0.0
    total_context_kl_cost = 0.0
    for batch in range(data_set.num_batches):
        unused_orig_x, x, s, c= data_set.get_batch(batch)
        feed = {model.input_data: x, model.sequence_lengths: s, model.c_expr: c}
        (cost, r_cost,
         kl_cost, context_kl_cost) = sess.run([model.cost, model.r_cost, model.kl_cost, model.context_kl_cost], feed)
        total_cost += cost
        total_r_cost += r_cost
        total_kl_cost += kl_cost
        total_context_kl_cost += context_kl_cost

    total_cost /= (data_set.num_batches)
    total_r_cost /= (data_set.num_batches)
    total_kl_cost /= (data_set.num_batches)
    total_context_kl_cost /= (data_set.num_batches)
    return (total_cost, total_r_cost, total_kl_cost, total_context_kl_cost)
...
def train(sess, model, eval_model, train_set, valid_set, test_set):
    ...
    for _ in range(hps.num_steps):
        ...
        _, x, s, c = train_set.random_batch()
        ...
        (train_cost, r_cost, kl_cost, context_kl_cost, _, train_step, _) = sess.run([
            model.cost, model.r_cost, model.kl_cost, model.context_kl_cost, model.final_state,
        ...
        if step % 20 == 0 and step > 0:
            ...
            c_summ = tf.summary.Summary()
            c_summ.value.add(tag='Context_KL_cost', simple_value=float(context_kl_cost))
            ...
            output_format = ('step: %d, lr: %.6f, klw: %0.4f, cost: %.4f, '
                           'recon: %.4f, kl: %.4f, context_kl: %.4f, train_time_taken: %.4f')
            output_values = (step, curr_learning_rate, curr_kl_weight, train_cost,
                           r_cost, kl_cost, context_kl_cost, time_taken)
            ...
            summary_writer.add_summary(c_summ, train_step)
        ...
        if step % hps.save_every == 0 and step > 0:
            ...
            (valid_cost, valid_r_cost, valid_kl_cost, valid_context_kl_cost) = evaluate_model(
            ...
            valid_c_summ = tf.summary.Summary()
            valid_c_summ.value.add(
              tag='Valid_context_kl_cost', simple_value=float(valid_context_kl_cost))
            ...
            output_format = ('best_valid_cost: %0.4f, valid_cost: %.4f, valid_recon: '
                           '%.4f, valid_kl: %.4f, valid_context_kl: %.4f, valid_time_taken: %.4f')
            output_values = (min(best_valid_cost, valid_cost), valid_cost,
                           valid_r_cost, valid_kl_cost, valid_context_kl_cost, time_taken_valid)
            output_log = output_format % output_values
            ...
            summary_writer.add_summary(valid_c_summ, train_step)
        ...
        if valid_cost < best_valid_cost:
            ...
            (eval_cost, eval_r_cost, eval_kl_cost, eval_context_kl_cost) = evaluate_model(
            ...
            eval_c_summ = tf.summary.Summary()
            eval_c_summ.value.add(
                tag='Eval_context_kl_cost', simple_value=float(eval_context_kl_cost))
            ...
            output_format = ('eval_cost: %.4f, eval_recon: %.4f, '
                             'eval_kl: %.4f, eval_context_kl: %.4f, eval_time_taken: %.4f')
            output_values = (eval_cost, eval_r_cost, eval_kl_cost, eval_context_kl_cost, time_taken_eval)
            output_log = output_format % output_values
            ...
            summary_writer.add_summary(eval_c_summ, train_step)
...
def trainer(model_params):
    ...
    np.set_printoptions(precision=8, edgeitems=6, linewidth=200, suppress=True) #Set printing options.
    ...
    reset_graph()
    model = sketch_rnn_model.Model(model_params,FLAGS.expr_num)
    eval_model = sketch_rnn_model.Model(eval_model_params,FLAGS.expr_num, reuse=True)
    sample_model = sketch_rnn_model.Model(sample_model_params,FLAGS.expr_num, reuse=True)
...
def console_entry_point():
    data = np.load('./dataset/abstract.npz', encoding='latin1')
